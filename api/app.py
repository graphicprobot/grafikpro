...
# === КЛАВИАТУРЫ ===
def master_menu():
    return {"keyboard": [["📊 Дашборд", "📅 Календарь"], ["📅 Расписание", "➕ Новая запись"], ["👥 Клиенты", "🔗 Моя ссылка"], ["📢 Свободные окна", "⚙️ Настройки"]], "resize_keyboard": True}
...
def get_smart_slots(master, date, service_name):
    ...
    for b in breaks:
        try:
            b_start_str, b_end_str = b.split("-")
            b_start = int(b_start_str.split(":")[0]) * 60
            b_end = int(b_end_str.split(":")[0]) * 60
...
def handle_calendar(chat_id):
    ...
    if not master: return send_message(chat_id, "Зарегистрируйтесь.")
    # Отправляем упрощенный HTML-календарь через WebApp ссылку
    html = f"""<html><head><meta charset='utf-8'><style>body{{font-family:sans-serif}} table{{border-collapse:collapse}} th,td{{border:1px solid #ccc;padding:4px;font-size:12px}} th{{background:#eee}}</style></head><body><h3>Расписание</h3><table><tr><th>Время</th>"""
    for i in range(7):
    ...
    for h in range(start_h, end_h + 1):
        ...
        for i in range(7):
            ...
            appts = [a for a in firestore_query("appointments", "master_id", "EQUAL", str(chat_id)) if a.get("date") == date and a.get("time", "").startswith(f"{h}:")]
            if appts:
                html += f"<td style='background:#90EE90'>{appts[0].get('client_name','?')[:6]}</td>"
            else:
                html += "<td></td>"
        html += "</tr>"
    html += "</table></body></html>"
    send_message(chat_id, "📅 *Веб-календарь*", reply_markup={"inline_keyboard": [[{"text": "Открыть календарь", "url": f"https://grafikpro.vercel.app/api/calendar?master={chat_id}"}]]})
...
def handle_reschedule_time(chat_id, appt_id, date, time):
    ...
    if appt.get("client_id"):
        send_message(int(appt["client_id"]), f"🔄 *Запись перенесена!*\n\n{appt.get('service')}\nНовое время: {date} в {time}")
...
def handle_repeat_appointment(chat_id, client_phone):
    ...
    app_link = f"tg://resolve?domain=grafikpro_bot&start=master_{link_id}"
    ...
    text = f"👋 Запишитесь снова на «{last_svc}»!\n\n{app_link}"
...
def handle_address_settings(chat_id):
    ...
    send_message(chat_id, f"📍 *Адрес*\nСейчас: {current}\n\nОтправьте новый адрес или метро:", reply_markup={"keyboard": [["🔙 Отмена"]], "resize_keyboard": True})
...
def handle_client_card(chat_id, phone):
    ...
    if not history: text += "Записей пока нет."
...
def handle_client_reschedule_start(chat_id, appt_id):
    ...
    STATES[str(chat_id)] = {"state": "client_reschedule_date", "appt_id": appt_id, "link_id": appt.get("master_id")}
...
def handle_client_reschedule_time(chat_id, appt_id, date, time):
    firestore_set("appointments", appt_id, {"date": date, "time": time, "reminded": False})
    appt = firestore_get("appointments", appt_id)
...
def handle_client_phone(chat_id, phone):
    ...
    send_message(chat_id, text, reply_markup=client_menu())
...
def handle_schedule_view(chat_id):
    ...
    for a in appointments:
        ...
        buttons = [[{"text": f"🔄 Перенести: {a.get('date')} {a.get('time')}", "callback_data": f"reschedule_{a['_id']}"}]]
    send_message(chat_id, text, reply_markup={"inline_keyboard": buttons})
...
def handle_services_settings(chat_id):
    ...
    if not master: return send_message(chat_id, "Сначала зарегистрируйтесь.")
    services = [s for s in master.get("services", []) if isinstance(s, dict) and s.get("name")]
    buttons = [[{"text": f"❌ {s['name']} ({s.get('price',0)}₽, {s.get('duration',60)}мин)", "callback_data": f"delservice_{s['name']}"}], [{"text": f"📋 Шаблон: {s['name']}", "callback_data": f"template_{s['name']}"}] for s in services]
...
def handle_free_slots(chat_id):
    ...
    text = f"🟢 *Окна на завтра:* {', '.join(free)}\n\n```Свободные окна: {', '.join(free)}\nЗапись: https://t.me/grafikpro_bot?start=master_{firestore_query('links', 'master_id', 'EQUAL', str(chat_id))[0]['_id']}```"
    send_message(chat_id, text)
...
def handle_manual_phone(chat_id, phone):
    ...
    buttons = [[{"text": f"{s['name']} ({s['price']}₽, {s.get('duration',60)}мин)", "callback_data": f"man_srv_{s['name']}"}] for s in services]
