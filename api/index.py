            ...
            if "document" in doc:
                ...
                for key, val in fields.items():
                    if "stringValue" in val:
                        ...
                        doc_data[key] = [v.get("stringValue", v.get("integerValue", "")) for v in vals]
...
def handle_schedule_settings(chat_id):
    ...
    send_message(chat_id, 
        ...
        f"Чтобы изменить, отправьте время в формате:\n`09:00-20:00`",
...
def handle_schedule_set(chat_id, text):
    try:
        if "-" not in text:
            send_message(chat_id, "❌ Неверный формат. Попробуйте: 09:00-20:00")
            return
        ...
        firestore_update("masters", str(chat_id), {"schedule": {"start": start, "end": end}})
        ...
        send_message(chat_id, "❌ Ошибка. Попробуйте ещё раз: 09:00-20:00")
...
def handle_schedule_view(chat_id):
    ...
    for appt in appointments:
        text += f"\n• {appt.get('date', '?')} в {appt.get('time', '?')} — {appt.get('service', '?')}"
...
def handle_client_service_select(chat_id, link_id, service_name):
    link = firestore_get("links", link_id)
    master = firestore_get(link.get("master_id"))
...
def handle_text(chat_id, user_name, username, text):
    ...
    # Если в режиме ожидания ввода времени
    if state == "setting_schedule":
        if text == "🔙 Отмена":
            STATES.pop(str(chat_id), None)
            send_message(chat_id, "❌ Отменено.", reply_markup=settings_menu())
        else:
            handle_schedule_set(chat_id, text)
        return
    
    # Если в режиме ввода услуги
    ...
    if text == "👤 Я мастер":
        ...
        handle_schedule_settings(chat_id)
        ...
        handle_schedule_view(chat_id)
    elif text == "➕ Новая запись":
        send_message(chat_id, "📝 Поделитесь вашей ссылкой с клиентом. Кнопка «🔗 Моя ссылка».")
    elif text == "👥 Клиенты":
        send_message(chat_id, "👥 Список клиентов появится после первых записей.")
    elif text == "📊 Статистика":
        send_message(chat_id, "📊 Статистика будет здесь после накопления данных.")
    else:
